...
def _GetDllPath():
    ...
    if (platform.architecture()[0]=='64bit'):
        return path.join(current_path, "ControlCANx64\ControlCAN.dll")
    elif (platform.architecture()[0]=='32bit'):
        return path.join(current_path, "ControlCANx86\ControlCAN.dll")
    else:
        raise Exception("未找到DLL文件.")
dll = ctypes.WinDLL(_GetDllPath())
...
class CanObj(Structure):
    ...
    def Load(self,ID,Data,Ext=None,Remote=None,SendType=None):
        ...
        self.SendType   = {0:0,1:1,2:2,3:3}.get(SendType,0)
...
class ZlgCan (object):
    ...
    def __init__(self, DeviceType, DeviceIndex,loggerName=None):
    ...
    def OpenDevice (self):
        rst = dll.VCI_OpenDevice(self.DeviceType, self.DeviceIndex, 0)
        if rst == 0:
            self.logger.warning("打开设备失败!")
            self._GetDLLErrCode(-1)
            return
        self.BoardInfo = _VCI_BOARD_INFO()
        rst = dll.VCI_ReadBoardInfo(self.DeviceType, self.DeviceIndex, byref(self.BoardInfo))
        if rst == 0:
            self.BoardInfo = None
    ...
    def CloseDevice (self):
        rst = dll.VCI_CloseDevice(self.DeviceType, self.DeviceIndex, 0)
        if rst == 0:
    ...
    def StartCan (self, BaudRate, Chx, Filter=None, mode=None):
        ...
        if self.DeviceType in [i.value for i in SpcDevice1]:
            baud = self.BaudRate.get(BaudRate.lower()).get("BTR_2eu")
            rst = dll.VCI_SetReference(self.DeviceType, self.DeviceIndex, Chx,0, byref(c_int(baud)))
            if rst == 0:
                self.logger.warning("配置波特率失败!")
                self._GetDLLErrCode(Chx)
                self.CloseDevice()
                return
        elif self.DeviceType in [i.value for i in SpcDevice2]:
            ...
            if rst == 0:
                ...
                self.CloseDevice()
                return
        elif self.DeviceType in [i.value for i in SpcDevice3]:
        ...
        vci.Mode = mode
        rst = dll.VCI_InitCAN(self.DeviceType, self.DeviceIndex, Chx,byref(vci))
        if rst == 0:
            self.logger.warning("初始化CAN卡失败！")
            self._GetDLLErrCode(Chx)
            return
        rst = dll.VCI_StartCAN(self.DeviceType, self.DeviceIndex, Chx)
        if rst == 0:
            self.logger.warning("打开CAN失败！")
            self._GetDLLErrCode(Chx)
            return

    def ResetCan (self, Chx):
    ...
    def Send(self, objs, Chx):
        ...
        rst = dll.VCI_Transmit(self.DeviceType, self.DeviceIndex, Chx, byref(msgs), len(msgs))
        if rst == 0:
    ...
    def Recv(self, Chx, Filter=None, WaitTime=None):
        ...
        if WaitTime==None:
            WaitTime = 200
        RecvSize = dll.VCI_GetReceiveNum(self.DeviceType, self.DeviceIndex, Chx)
        if RecvSize==0:
            return tuple()
        rst = dll.VCI_Receive(self.DeviceType, self.DeviceIndex, Chx, \
    ...
    def ClearRecvBuffer (self, Chx):
        ...
        rst = dll.VCI_ClearBuffer(self.DeviceType, self.DeviceIndex, Chx)
        if rst == 0:
    ...
    def _GetDLLErrCode(self, Chx):
        err = _VCI_ERR_INFO()
        dll.VCI_ReadErrInfo(self.DeviceType, self.DeviceIndex, Chx, byref(err))
